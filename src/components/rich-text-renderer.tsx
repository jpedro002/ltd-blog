...
export function RichTextRenderer({ content }: RichTextRendererProps) {
  ...
  const options = {
    ...
    renderNode: {
      ...
      [BLOCKS.QUOTE]: (node: any, children: React.ReactNode) => (
        <blockquote className="border-l-4 border-purple-500 pl-4 py-1 mb-4 italic">{children}</blockquote>
      ),
      ...
      [BLOCKS.CODE]: (node: any) => {
        ...
        return (
          <div className="mb-6">
            <SyntaxHighlighter language={language} style={atomDark} className="rounded-md">
        ...
        if (entry.sys.contentType.sys.id === "blogPost") {
          return (
            <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg my-6">
              ...
              <Link href={`/post/${entry.fields.slug}`} className="text-purple-600 hover:underline">
                {entry.fields.title}
              </Link>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{entry.fields.excerpt}</p>
            </div>
          )
        }
  ...
  return <div suppressHydrationWarning className="rich-text">{documentToReactComponents(content, options)}</div>
}
